...
export class ConfigService {
    ...
    public loadAppSettings(): AppConfig {
    ...
    public GetConfig(): Observable<MainConfig> {
        return this.httpClient.get(`${this.controllerURL}/GetConfig`).pipe(
            map((ii: unknown) => {
    ...
    public SaveConfig(mainConfig: MainConfig): Observable<boolean> {
        return this.httpClient
            .post(`${this.controllerURL}/SaveConfig`, mainConfig)
            .pipe(
                map((ii: unknown) => {
                    return ii as boolean
                })
    ...
    public GetFolders(): Observable<FolderLibrary[]> {
        return this.httpClient.get(`${this.controllerURL}/GetFolders`).pipe(
            map((ii: unknown) => {
    ...
    public SaveFolders(folders: FolderLibrary[]): Observable<boolean> {
        return this.httpClient
            .post(`${this.controllerURL}/SaveFolders`, folders)
            .pipe(
                map((ii: unknown) => {
                    return ii as boolean
                })
